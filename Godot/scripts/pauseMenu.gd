...
func _ready() -> void:
	...
	$PanelContainer2/VBoxContainer/AntiOption.select(0)
	$PanelContainer2/VBoxContainer/HSlider.value = 1.00
	$PanelContainer2/VBoxContainer/VsyncOption.select(1)
...
func _on_anti_option_item_selected(index: int) -> void:
	# Set proper MSAA setting
	if index == 0: #MSAA disabled
		get_viewport().msaa_3d = 0
	elif index == 1: #MSAA 2X
		get_viewport().msaa_3d = 1
	elif index == 2: #MSAA 4X
		get_viewport().msaa_3d = 2
	elif index == 3: #MSAA 8X
